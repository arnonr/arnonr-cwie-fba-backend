...
const methods = {
    scopeSearch(req, limit, offset) {
        ...
        if (req.query.prefix_id) $where["prefix_id"] = req.query.prefix_id;

        if (req.query.prefix_name)
            $where["prefix_name"] = {
                [Op.like]: "%" + req.query.prefix_name + "%",
            };
    ...
    insert(data) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const res = methods.findById(inserted.prefix_id);
    ...
    update(id, data) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // Update
                // data.prefix_id  = parseInt(id);
                await db.update(data, { where: { prefix_id: id } });
                let res = methods.findById(obj.prefix_id);
